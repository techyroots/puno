...
const token_address = "TQtcmoNVv8CkQErCQ13RUEuedhHUCmXELr";
const contractaddress = "TCWLnnNeAF4mbv9xLjGmdCo7ACJakx5eJR";
                ...
                if (!email_check.length) {
                    if (email_otp != '' && email_otp != null && email_otp != undefined) {
                        ...
                        if (email_otp_result['success']) {
                            ...
                            console.log("hfhdhd", check_refferal_pause[0].ref_pause);
                                ...
                                if (check_refferal_pause[0].ref_pause == "false") {
                                    if (referral != '' && referral != null && referral != undefined) {
                                        ...
                                        if (check_referral.length) {
                                            ...
                                            await db.query(`INSERT INTO transactions (user_id, wallet_address, type, tron_amount, token_amount, fromUser) VALUES (${check_referral[0].id}, '${check_referral[0].wallet_address}', 'Received Referral Bonus', 0, 1, '${username}')`);
...
exports.withdrawal = async(req, res) => {
    ...
    let valid = WAValidator.validate(wallet_address, 'trx');
    if (valid) {
        let user_details = await db.query("SELECT * from users WHERE id =" + user_id);
        let refBonus = 0,
            ref = 0;
        ...
        if (user_details.length) {
            ...
            if (type == 'puno' && wallet_address != null) {
                    ...
                    setTimeout(async function() {
                        ...
                        if (events != '') {
                            ...
                            if (transaction.receipt.result == "SUCCESS") {
                                ...
                                if (sql) {
                                    if (wallet_address != user_details[0].wallet_address) {
                ...
                for (let i = 0; i < referalData.length; i++) {
                    let referalList = await db.query(`SELECT * from transactions WHERE type = 'Invest' AND wallet_address = '${referalData[i].wallet_address}'`);
                    console.log("gghh", referalList);
                ...
                if (user_details[0].referral_bonus > 0) {
                    if (user_details[0].referral_bonus >= amount && amount >= 10) {
                        ...
                        if (user_details[0].referral_bonus - refBonus > amount) {
                            let instance1 = await rewardtronGrid.contract().at(token_address);
                            let transfer = await instance1.transfer(wallet_address, amount * 10 ** 6).send();
                            let events = await rewardtronGrid.getEventResult(token_address, { eventName: "Transfer", size: 1 });
                            if (events) {
                                let transaction = await tronGrid.trx.getUnconfirmedTransactionInfo(events[0].transaction);
        ...
        res.status(200).send({ success: false, msg: 'You entered invalid address', data: {}, errors: '' });
    }
}
...
exports.transfer = async(req, res) => {
    ...
    let valid = WAValidator.validate(transferAddress, 'trx');
    if (valid) {
        let user_details = await db.query("SELECT * FROM users WHERE id = " + user_id);
        if (user_details.length) {
            let trongrid = new TronWeb(fullNode, solidityNode, eventServer, user_details[0].private_key);
...
exports.redeem = async(req, res) => {
    try {
        ...
        let valid = WAValidator.validate(walletAddress, 'trx');
        if (valid) {
            let instance = await tronGrid.contract().at(token_address);
            await instance.redeem(walletAddress, amount * 10 ** 6).send().then(
                setTimeout(async function() {
                    await db.query(`INSERT INTO transactions (user_id, wallet_address, to_wallet, type, tron_amount, token_amount) VALUES ( 0 ,'${token_address}', '${walletAddress}', 'Redeem Locked Tokens', 0, ${amount})`);
                    await db.query(`INSERT INTO transactions (user_id, wallet_address, to_wallet, type, tron_amount, token_amount, fromUser) VALUES ( 0,'${walletAddress}',  'null', 'Received Locked Tokens', 0, ${amount}, '${token_address}')`);
